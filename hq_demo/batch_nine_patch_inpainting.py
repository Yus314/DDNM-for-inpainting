...
def save_patch_results(patches: List[torch.Tensor], output_dir: Path):
    ...
    for i, patch in enumerate(patches):
        patch_image = tensor_to_pil(patch)
...
def process_single_image_nine_patch(
    ...
    try:
        ...
        # 画像とマスクの読み込み（utilities使用）
        gt, pil_image = load_image_as_tensor(str(image_path), image_size=512, device=device)
        gt_keep_mask, arr_mask = load_mask_as_tensor(mask_path, mask_size=512, device=device)
        ...
        result_image = tensor_to_pil(final_result)
        ...
        masked_input = tensor_to_pil(gt * gt_keep_mask)
        masked_input.save(output_dir / "masked_input.png")

        # マスクの保存（arr_maskの最初のチャンネルのみ使用）
        mask_image = Image.fromarray((arr_mask[:, :, 0] * 255).astype(np.uint8))
